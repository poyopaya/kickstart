...
typeOut = (str, container, cb, startWith) ->
  ...
  @interval = setInterval ->
    i++
    _str = str.substr(0, i) 
    container.innerHTML = startWith + _str.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\|/g, '<br>')
    if i > str.length - 1
...
slide2 = ->
  ...
  slide2TO = setTimeout ->
    $button.classList.remove('button')
    $button.classList.remove('button-primary')
    typeOut '<button class="cta">Call to Action</button>||.cta {|  @include button();|}', k$.$('#source'), slide3, "&lt;!-- OR define your own --&gt;<br><br>"
