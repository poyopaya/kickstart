...
describe('k$.dropdown', function() {
  it('should not apply to elements without uls', function() {
    expect(k$.$$('#ddbutton3.with-submenu').length).to.equal(0);
    return expect(k$.$$('#ddbutton4.with-submenu').length).to.equal(0);
  });
  it('should apply to elements with "role=button"', function() {
    console.log(k$.$$("[role='button']").length);
    return assert(k$.$('#ddbutton1').classList.contains('with-submenu'), '.with-submenu found');
  });
  return it('should apply to button elements', function() {
